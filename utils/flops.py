...
def count_modelres_param_flops(model=None, input_res=32, multiply_adds=True):
    list_strip = []
    ...
    def foo(net):
        ...
        if not childrens:
            if isinstance(net, FilterStripe_res):
...
def count_modelvgg_param_flops(model=None, input_res=32, multiply_adds=True):
    list_strip = []
    ...
    def foo(net):
        childrens = list(net.children())
        if not childrens:
            if isinstance(net, FilterStripe_vgg):
                net.register_forward_hook(filter_strip_hook)
            if isinstance(net, Linear):
                net.register_forward_hook(linear_hook)
            if isinstance(net, BatchNorm):
